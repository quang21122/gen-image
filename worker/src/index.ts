...
function jsonResponse(data: any, status: number = 200, origin: string | null, allowedOrigins: string): Response {
...
async function handleImageGeneration(request: Request, origin: string | null, env: Env): Promise<Response> {
  try {
...
async function handleTranslation(request: Request, origin: string | null, env: Env): Promise<Response> {
  try {
    const body = await request.json();
